...
class Resp:
    def __init__(self, text, headers=None):
        ...
        self.retrieved_at = pd.Timestamp.utcnow().to_pydatetime()
...
def test_statcan_vector_metadata_and_fetch():
    info = (
        '[{"status":"SUCCESS","object":{"vectorId":123,'
        '"SeriesTitleEn":"Test series","frequencyCode":9}}]'
    )
    data = (
        '{"status":"SUCCESS","object":[{"vectorId":123,"vectorDataPoint":'
        '[{"refPer":"2020-01-01","value":10},{"refPer":"2021-01-01","value":11}]}]}'
    )
    base = "https://www150.statcan.gc.ca/t1/wds/rest"
    c = FakeClient(
        {
            f"{base}/getSeriesInfoFromVector": Resp(info),
            f"{base}/getDataFromVectorByReferencePeriodRange": Resp(data),
        }
    )
...
def test_ilostat_csv_normalization():
    ...
    csv = (
        "ref_area,ref_area.label,indicator,indicator.label,time,obs_value\n"
...
def test_sdmx_requires_flow_and_key():
    ...
    try:
        ...
        assert "DATAFLOW|KEY" in str(exc)
    else:
...
def test_faostat_is_explicitly_limited_without_verified_auth_flow():
    ...
    try:
        ...
        assert "intentionally not enabled" in str(exc)
    else:
        raise AssertionError("Expected explicit FAOSTAT limitation")
