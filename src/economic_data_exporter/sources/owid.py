...
class OwidSource(DataSource):
    ...
    def search(self, query: str, *, options: dict[str, object] | None = None) -> list[SeriesMetadata]:
        # Exact chart-slug lookup is stable and documented. Broad OWID search APIs are marked active-development.
        ...
        for column_id, item in columns.items():
            results.append(
                SeriesMetadata(
                    source=self.name,
                    series_id=slug,
                    name=str(item.get("titleShort") or item.get("title") or chart.get("title") or column_id),
                    frequency=str(item.get("timespan") or "Chart-defined"),
                    units=str(item.get("unit") or item.get("shortUnit") or ""),
                    notes=str(item.get("descriptionShort") or item.get("description") or chart.get("subtitle") or ""),
                    source_url=f"{self.base_url}/{slug}",
                    attribution=str(item.get("citationFull") or chart.get("citation") or "Our World in Data and underlying provider"),
                    license_text="OWID-produced data is CC BY; third-party source licenses shown in chart metadata also apply.",
        ...
        if not results:
            results.append(
                SeriesMetadata(
                    ...
                    source_url=f"{self.base_url}/{slug}",
                    attribution=str(chart.get("citation") or "Our World in Data and underlying provider"),
                    license_text="OWID-produced data is CC BY; third-party source licenses shown in chart metadata also apply.",
                )
            )
        return results
    ...
    def fetch(self, request: SeriesRequest, *, cancel: CancelCheck) -> SourceResult:
        ...
        id_columns = [column for column in ("Entity", "Code", time_column) if column in wide.columns]
        ...
        if requested_column:
            if requested_column not in value_columns:
                raise ParsingError(f"OWID data column '{requested_column}' is not present in this chart.")
            value_columns = [requested_column]
        if not value_columns:
            raise ParsingError("OWID chart contains no data columns.")
        if len(value_columns) > 1:
            raise ParsingError(
                "OWID chart contains multiple data columns. Use Validate slug metadata and select one column explicitly."
            )
        long = wide.melt(id_vars=id_columns, value_vars=value_columns, var_name="column", value_name="value")
        geography_filter = request.geography.strip()
        if geography_filter:
            code_match = long.get("Code", pd.Series(index=long.index, dtype="object")).astype(str).str.upper() == geography_filter.upper()
            name_match = long["Entity"].astype(str).str.casefold() == geography_filter.casefold()
            long = long[code_match | name_match]
        if time_column == "Year":
            long["date"] = pd.to_datetime(long[time_column].astype("Int64").astype(str) + "-01-01", errors="coerce")
        else:
        ...
        units = sorted({str((columns_meta.get(column) or {}).get("unit") or "") for column in value_columns} - {""})
        citations = sorted({str((columns_meta.get(column) or {}).get("citationFull") or "") for column in value_columns} - {""})
        selected_meta = columns_meta.get(value_columns[0]) or {}
        metadata = SeriesMetadata(
            source=self.name,
            series_id=slug,
            name=str(selected_meta.get("titleShort") or selected_meta.get("title") or chart.get("title") or column_names or slug),
            geography=geography_filter,
            frequency="Annual" if time_column == "Year" else "Daily/chart-defined",
            units="; ".join(units),
            notes=str(chart.get("subtitle") or "") + (f" Columns: {column_names}." if len(value_columns) > 1 else ""),
            source_url=f"{self.base_url}/{slug}",
            attribution="; ".join(citations) or str(chart.get("citation") or "Our World in Data and underlying provider"),
            license_text="OWID-produced data is CC BY; third-party source licenses shown in chart metadata also apply.",
        )
