...
class SDMXSource(DataSource):
    ...
    def search(self, query: str, *, options: dict[str, object] | None = None) -> list[SeriesMetadata]:
        ...
        if "|" in query:
            ...
            return [SeriesMetadata(source=self.name, series_id=f"{flow}|{key}", name=query.strip(), source_url=self.source_url, attribution=self.attribution, license_text=self.license_text)]
        ...
        try:
            ...
            raise ParsingError(f"{self.name} dataflow catalogue was not a readable CSV response.") from exc
        if frame.empty:
            return []
        text = frame.fillna("").astype(str)
        mask = text.apply(lambda row: q in " ".join(row.tolist()).casefold(), axis=1) if q else pd.Series(True, index=frame.index)
        ...
        for _, row in frame.loc[mask].head(100).iterrows():
            ...
            results.append(SeriesMetadata(source=self.name, series_id=f"{flow_ref}|", name=label, notes=self.id_help, source_url=self.source_url, attribution=self.attribution, license_text=self.license_text, extra={"selectable": False}))
        return results
    ...
    def _csv_data(self, response_text: str) -> pd.DataFrame:
        ...
        if frame.empty:
            return frame
        value_col = next((c for c in frame.columns if str(c).upper() in {"OBS_VALUE", "OBS_VALUE_LABEL", "VALUE"}), None)
        time_col = next((c for c in frame.columns if str(c).upper() in {"TIME_PERIOD", "TIME"}), None)
        if value_col is None or time_col is None:
            raise ParsingError(f"{self.name} SDMX response lacks TIME_PERIOD/OBS_VALUE columns.")
        geography_col = next((c for c in frame.columns if str(c).upper() in {"REF_AREA", "REF_AREA_LABEL", "GEO", "GEO_LABEL"}), None)
        out = pd.DataFrame({"date": pd.to_datetime(frame[time_col], errors="coerce"), "value": pd.to_numeric(frame[value_col], errors="coerce")})
        ...
        for series in root.iter():
            ...
            for child in series:
                if child.tag.endswith("SeriesKey"):
                    for value in child:
                        if value.attrib.get("id", "").upper() in {"REF_AREA", "GEO"}:
                            geo = value.attrib.get("value", "")
    ...
    def fetch(self, request: SeriesRequest, *, cancel: CancelCheck) -> SourceResult:
        ...
        try:
            if "xml" in response.headers.get("Content-Type", "").lower() or response.text.lstrip().startswith("<"):
        ...
        metadata = SeriesMetadata(source=self.name, series_id=series_id, name=series_id, geography=request.geography, frequency=request.frequency, source_url=self.source_url, attribution=self.attribution, license_text=self.license_text, notes="Retrieved through the provider's documented SDMX REST service; no resampling or unit conversion applied.")
        return finish_result(request=request, metadata=metadata, dates=frame["date"], values=frame["value"], geographies=frame["geography"], retrieved_at=response.retrieved_at, from_cache=response.from_cache)
